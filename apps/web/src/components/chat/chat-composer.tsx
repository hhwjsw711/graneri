...
export function ChatComposer({
	...
	const [queuePreviewIds, setQueuePreviewIds] = React.useState(() =>
		HARD_CODED_QUEUE_PREVIEW_FOLLOW_UPS.map(
			(queuedFollowUp) => queuedFollowUp.id,
		),
	);
	const shouldShowQueuePreview =
		import.meta.env.DEV && showQueuePreview && queuedFollowUps.length === 0;
	const hardCodedQueuedFollowUps = React.useMemo<
	...
	>(() => {
		...
		const previewById = new Map(
			HARD_CODED_QUEUE_PREVIEW_FOLLOW_UPS.map((queuedFollowUp) => [
				queuedFollowUp.id,
				queuedFollowUp,
			]),
		);

		return queuePreviewIds
			.map((id) => previewById.get(id))
			.filter(
				(
					queuedFollowUp,
				): queuedFollowUp is NonNullable<typeof queuedFollowUp> =>
					Boolean(queuedFollowUp),
			)
			.map((queuedFollowUp) => ({
				...queuedFollowUp,
				isDeleting: false,
				...
				isSendingNow: false,
				onDelete: () =>
					setQueuePreviewIds((ids) =>
						ids.filter(
							(queuedFollowUpId) => queuedFollowUpId !== queuedFollowUp.id,
						),
					),
				onEdit: () => {
					onDraftChange(queuedFollowUp.text);
					setQueuePreviewIds((ids) =>
						ids.filter(
							(queuedFollowUpId) => queuedFollowUpId !== queuedFollowUp.id,
						),
	...
	}, [onDraftChange, queuePreviewIds, shouldShowQueuePreview]);
	const visibleQueuedFollowUps = shouldShowQueuePreview
		? hardCodedQueuedFollowUps
		: queuedFollowUps;
	const handleVisibleQueuedFollowUpsReorder = shouldShowQueuePreview
		? setQueuePreviewIds
		: onQueuedFollowUpsReorder;
	...
	return (
		...
		>
			...
			/>
			{visibleQueuedFollowUps.length > 0 ? (
				<ChatComposerQueuedFollowUpBar
					queuedFollowUps={visibleQueuedFollowUps}
					onReorder={handleVisibleQueuedFollowUpsReorder}
				/>
			) : null}
			<InputGroup
...
const HARD_CODED_QUEUE_PREVIEW_FOLLOW_UPS = [
	{
		id: "queue-preview-1",
		text: "first queued: summarize in 2 bullets.",
	},
	{
		id: "queue-preview-2",
		text: "make the answer shorter and keep the headings.",
	},
	{
		id: "queue-preview-3",
		text: "add one practical example at the end.",
	},
] satisfies Array<{ id: string; text: string }>;
...
function ChatComposerQueuedFollowUpBar({
	onReorder,
	queuedFollowUps,
}: {
	...
	return (
		...
		>
			<SortableContext items={ids} strategy={verticalListSortingStrategy}>
				<ul className="mx-auto w-[calc(100%-1rem)] max-w-[548px] overflow-hidden rounded-t-lg rounded-b-none bg-transparent text-sm">
					{ids.map((id) => {
		...
		>
			...
			<div className="flex shrink-0 items-center gap-1">
				<button
					...
					className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-md px-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-default"
				>
