...
export default defineSchema({
	...
	assistantAgents: defineTable({
		...
		lastTaskMessage: v.optional(v.string()),
		...
		.index("by_rootChatId_and_agentPath", ["rootChatId", "agentPath"])
		.index("by_rootChatId_and_status", ["rootChatId", "status"])
		.index("by_parentAgentId", ["parentAgentId"])
		.index("by_activeRunId", ["activeRunId"]),
	assistantAgentMailbox: defineTable({
		ownerTokenIdentifier: v.string(),
		workspaceId: v.id("workspaces"),
		rootChatId: v.id("chats"),
		...
		.index("by_receiverAgentId_and_status_and_createdAt", [
			"receiverAgentId",
			"status",
			"createdAt",
		])
		.index("by_rootChatId_and_status_and_createdAt", [
			"rootChatId",
			"status",
			"createdAt",
		])
		.index("by_sourceCallId", ["sourceCallId"]),
